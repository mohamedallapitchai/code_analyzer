...
- The input is a grid of `N` lines, each of length `M`, with `N ≥ 1`, `M ≥ 1`.
- Allowed characters:
  - `'*'` for a filled cell
  - `'-'` for an empty cell
- Validation rules:
...
- Error handling: produce a clear error message when validation fails.
...
- Area between two points `(r1, c1)` and `(r2, c2)`:
  - `b = r2 − r1`
  - `l = c2 − c1`
  - `area = b × l`
...
4. Filter by non-overlapping bounding rectangles
   - For every pair of components, compute bounding rectangles.
   - Two rectangles are non-overlapping if any of the following holds:
     - `R.right <= S.left`
     - `S.right <= R.left`
     - `R.bottom <= S.top`
     - `R.top >= S.bottom`
...
5. Compute each remaining component’s best area
   ...
   - If multiple points yield the same best area, select the representative opposite corner using row-then-column order (smallest row, then smallest column).

## Global Selection and Zero-Area Rule
...
No overlaps among rectangles → all remain.  
...
```
Components:
- H: rows 1..2, cols 1..2; anchor `(1, 1)`; best area = 1 at `(2, 2)`
- I: rows 1..2, cols 4..6; anchor `(1, 4)`; best area = 2 at `(2, 6)`
- J: rows 3..4, cols 4..4; anchor `(3, 4)`; best area = 0

No overlaps among rectangles.  
...
## I/O Details (for the implementation)
